...
def generate_output(arm_angle, spine_angle, wrist_head_dist, rounded_center):
                ...
                if rounded_center:
                    rounded_center = "  Body Center: " + str(rounded_center)
                else:
...
objects_tolist=''
count=0
measure_objects=['Bowl','Laptop','Bottle','Keyboard','Cup','Book','Tennis racket']
track_objects=pd.DataFrame(columns=['time','cup','book'])
...
#Video write
    ...
    if (args.save_video==True):
        ...
        out= cv2.VideoWriter(args.video_file,cv2.VideoWriter_fourcc('M','J','P','G'),30,(frame_width,frame_height))
    count = 0
    count_f=0
    frames=0
    font_title=ImageFont.truetype('helvetica-bold.ttf',round(frame_width*0.015))
    ...
    while True:
        objects_tolist=''#set variable to 0
        for obj in measure_objects:   
            #vars()[obj+'s']=[]
...
#--------------------- OBJECT ANALYSIS ----------------------------------------
            ...
            if results:
                ...
                for cat, score, bounds in results:
                    ...
                    #print(objects)
                    if cat.decode("utf-8") in 'bottle':
                        if saved:
                            #print(saved)
                            for obj in saved:
                                #print(obj[0])
                                if ('bottle' not in obj[0]):  
                                    count=0
                                    save_object()
                                else:
                                     count+=1
...
##Object
              ...
              for cat, score, bounds in results:
                x, y, w, h = bounds
                draw.text((int(x-w/2), int(y-h/2)-round(frame_width*0.021)), str(cat.decode("utf-8")),font=font_objects)
...
##Saved Object
              if (saved):
                for cat, score, bounds in saved:
                   x, y, w, h = bounds
                   cv2.rectangle(image, (int(x-w/2),int(y-h/2)),(int(x+w/2),int(y+h/2)),(55,0,0))
                   cv2.putText(image,(cat), (int(x-w/2), int(y-h/2)), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 255))
...
#              test=image-image
            ...
            #        (10, 15),  cv2.FONT_HERSHEY_COMPLEX, 0.5,
             #       (255, 255, 255), 2)
